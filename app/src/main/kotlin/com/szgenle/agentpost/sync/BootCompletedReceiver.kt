 ...
 * 广播重新拉起 [PushSyncService]。仅当用户打开了「实时推送」开关时才启动，
 * 不会额外耗电。
 */
class BootCompletedReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        if (realtimePush) {
            AppLog.i(TAG, "restarting PushSyncService")
            PushSyncService.start(context)
        } else {
            AppLog.i(TAG, "realtime push off, skip starting service")
        }
    }
